...
-- Users table for authentication (LoginPage, SignUpPage, AdminPage)
...
-- Donees table (beneficiaries) - DoneePage, DoneeContactListPage
...
-- Donations table - AllDonationsPage, PayDonationsPage
...
-- Cases table for case tracking - CaseTrackingPage, CaseTrackingReportsPage
...
-- CaseUpdates table - CaseTrackingPage
...
-- Chat/Messages table for employee communication - EmployeeChatPage
...
-- Dashboard statistics table - DashboardPage
IF OBJECT_ID(N'dbo.DashboardStats', N'U') IS NULL
BEGIN
    PRINT 'Creating table dbo.DashboardStats...';
    CREATE TABLE dbo.DashboardStats (
        Id           INT IDENTITY(1,1) PRIMARY KEY,
...
-- Reports table - ReportsPage
IF OBJECT_ID(N'dbo.Reports', N'U') IS NULL
BEGIN
    PRINT 'Creating table dbo.Reports...';
    CREATE TABLE dbo.Reports (
        ...
        CONSTRAINT FK_Reports_Users
            FOREIGN KEY (GeneratedBy) REFERENCES dbo.Users(Id)
    );
END
GO
...
-- System Settings table - SystemSettingsPage
...
MERGE dbo.DashboardStats AS target
USING (VALUES
    (N'Total Donees', 0, CAST(GETDATE() AS DATE)),
    (N'Total Donations', 0, CAST(GETDATE() AS DATE)),
    (N'Active Cases', 0, CAST(GETDATE() AS DATE)),
    (N'Pending Inquiries', 0, CAST(GETDATE() AS DATE))
) AS src(StatName, StatValue, StatDate)
ON target.StatName = src.StatName AND target.StatDate = src.StatDate
WHEN MATCHED THEN UPDATE SET StatValue = src.StatValue, UpdatedAt = SYSUTCDATETIME()
WHEN NOT MATCHED THEN INSERT (StatName, StatValue, StatDate, UpdatedAt)
VALUES (src.StatName, src.StatValue, src.StatDate, SYSUTCDATETIME());
